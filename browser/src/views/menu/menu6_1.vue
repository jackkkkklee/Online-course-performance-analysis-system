  ...
  methods: {
    ...
    createData() {
      this.$refs["dataForm"].validate((valid) => {
        ...
        if (flag) {
          ...
          // });
          courseApi
            .addCourse(this.temp.cname, this.temp.startTime, this.temp.endTime)
            .then((res) => {
              this.fetchData();
            });
          this.dialogFormVisible = false;
    ...
    updateData() {
      this.$refs["dataForm"].validate((valid) => {
        ...
        // 调用接口
        courseApi
          .updateCourse(this.temp.cname, this.temp.startTime, this.temp.endTime)
          .then((res) => {
            this.fetchData();
          });
        this.dialogFormVisible = false;
    ...
    handleDelete(idx, row) {
      this.$confirm("Are you sure you want to delete this course?", "Tips", {
        ...
        .then(() => {
          ...
          courseApi.deleteCourse(row.course).then((res) => {
            this.fetchData();
          });
          this.$message.success("Delete successfully");
        })
